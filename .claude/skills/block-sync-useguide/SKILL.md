...
块文件放 `.claude/rules/architecture/`，按代码库目录结构镜像建文件夹：

```
代码库:  claude-driver/src/main/services/
规则镜像: .claude/rules/architecture/src/main/services.md

代码库:  claude-driver/src/renderer/
规则镜像: .claude/rules/architecture/src/renderer.md
```

> 顶层 `docs/architecture.md` 和 `docs/TDD.md` 不在 `.claude/rules/` 下（它们是汇总文档，不是 claude code rule）。

### 2.2 块文件格式（每个块文件 = claude code rule + block_sync 块）
...
**规则**：
- **frontmatter `paths`**：指向该块对应的代码文件夹 glob（如 `claude-driver/src/main/**/*`），相对项目根。**顶层 `architecture.md` 和 `TDD.md` 不加 paths frontmatter**。
...
| 体系 | 模板来源 | 章节结构 |
|------|---------|---------|
| Architecture | `.claude/skills/docs-generator/references/architecture-template.md` | Overall Architecture, Module Boundary, Component Interaction, Data Flow, Technology Selection, Non-functional Design |
| TDD | `.claude/skills/docs-generator/references/tdd-template.md` | Module Overview, API, Data Model, Sequence, State Machine, Exception, Monitoring, Testing |
...
2. **建文档骨架**：
   ...
   - 创建 `.claude/rules/architecture/` 镜像目录与块文件（frontmatter paths + template 内容 + 末尾指向）。
...
- 顶层两文档无 `paths` frontmatter（不是 claude code rule）。
