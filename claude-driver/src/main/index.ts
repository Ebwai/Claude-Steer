...
import type { ClaimStatus, PermissionMode, Project, GitMark, HookPayload, HookEvent } from '../shared/types/index'
...
// 独立通知窗口（单例，不设 parent，alwaysOnTop 可配置，关闭=隐藏到托盘）
let notificationWindow: BrowserWindow | null = null
let isAppQuitting = false

/** 打开或聚焦独立通知窗口（幂等，窗口已存在则聚焦） */
function openNotificationWindow(): void {
  ...
  const alwaysOnTop = config.notifWindowAlwaysOnTop !== false  // 默认 true
  const preloadPath = join(__dirname, '../preload/index.js')
  notificationWindow = new BrowserWindow({
    width: 600, height: 600,
    minWidth: 400, minHeight: 400,
    title: '🔔 通知 — Claude Steer',
  ...
  // 关闭 = 隐藏到托盘（不销毁），应用退出时真正关闭
  notificationWindow.on('close', (e) => {
    if (!isAppQuitting && notificationWindow) {
      e.preventDefault()
  ...
  if (is.dev && process.env['ELECTRON_RENDERER_URL']) {
    notificationWindow.loadURL(`${process.env['ELECTRON_RENDERER_URL']}#/notifications`)
  } else {
    notificationWindow.loadFile(join(__dirname, '../renderer/index.html'), { hash: '/notifications' })
  }
  // Catch-up：通知窗口加载后，发送当前所有活跃 session 的 PTY_BIND 事件
  // 确保通知窗口知道已存在的 session（用于项目分割和请求分组）
  setTimeout(() => {
    if (!notificationWindow || notificationWindow.isDestroyed()) return
    for (const [claudeId, { ptyId, cwd }] of sessionCwdMap.entries()) {
      notificationWindow.webContents.send(IPC.PTY_BIND, { ptyId, claudeId, transcriptPath: null, cwd })
    }
    console.log(`[notification] catch-up: sent ${sessionCwdMap.size} PTY_BIND events`)
    // 重放最近的 PermissionRequest（通知窗口加载前广播的事件可能因 handler 未注册而错过）
    for (const p of recentPermissionPayloads) {
      const event: HookEvent = {
        eventName: p.hook_event_name,
      ...
      notificationWindow.webContents.send(IPC.HOOK_EVENT, event)
    }
    console.log(`[notification] catch-up: replayed ${recentPermissionPayloads.length} PermissionRequest events`)
  }, 1500) // 等待 renderer 初始化（handler 工厂注册）
  console.log('[notification] window opened')
}

/** 向所有活跃渲染进程广播事件（mainWindow + notificationWindow） */
function sendToRenderers(channel: string, data: unknown): void {
...
// 同时匹配正常空格和压缩文本（终端可能压缩空格：Isthisaprojectyoucreatedoroneyoutrust?）
const TRUST_DIALOG_RE = /trust(?:the|these|this)?\s*(?:files|folder|directory|project)|do\s*you\s*trust|i\s*trust\s*this|trust\s*folder/i
...
// 存储当前活跃 session 的 cwd，用于通知窗口 catch-up（通知窗口打开时重放 PTY_BIND）
const sessionCwdMap = new Map<string, { ptyId: string; cwd: string }>()
// 缓存最近的 PermissionRequest Hook payload，用于通知窗口 catch-up（通知窗口加载完成时重放）
const recentPermissionPayloads: HookPayload[] = []
const MAX_RECENT_PERMISSION = 100
...
function bindPtyToClaudeSession(ptyId: string, claudeId: string, transcriptPath: string, cwd: string): void {
  ...
  sessionCwdMap.set(claudeId, { ptyId, cwd })
  const bindPayload = { ptyId, claudeId, transcriptPath, cwd }
  mainWindow?.webContents.send(IPC.PTY_BIND, bindPayload)
  if (notificationWindow && !notificationWindow.isDestroyed()) {
    notificationWindow.webContents.send(IPC.PTY_BIND, bindPayload)
  }
...
function unbindPtyFromClaudeSession(claudeId: string): void {
  ...
  sessionCwdMap.delete(claudeId)
  const unbindPayload = { ptyId, claudeId }
  mainWindow?.webContents.send(IPC.PTY_UNBIND, unbindPayload)
  if (notificationWindow && !notificationWindow.isDestroyed()) {
    notificationWindow.webContents.send(IPC.PTY_UNBIND, unbindPayload)
  }
...
function registerIpcHandlers(): void {
    ...
    try {
      ...
      ptyManager.startSession({
        ...
        onData: (sid, data) => {
          ...
          sendToRenderers(IPC.SESSION_STATUS, { sessionId: sid, data })
        ...
        onExit: (sid, exitCode) => {
          ...
          sendToRenderers(IPC.SESSION_STATUS, {
            sessionId: sid,
            status: 'Completed',
          })
          // PTY 退出时直接向渲染进程发 PTY_UNBIND，不依赖 SessionEnd Hook
          // 注意：不能用 unbindPtyFromClaudeSession(sid)，因为迁移后 claudeToPtyMap 的 key
          // 是真实 claudeId 而非 sid（ptyId），lookup 会失败导致 early return
          const claudeId = ptyToClaudeMap.get(sid) ?? sid
          sendToRenderers(IPC.PTY_UNBIND, { ptyId: sid, claudeId })
          unbindPtyFromClaudeSession(claudeId)
      ...
      sessionCwdMap.set(sessionId, { ptyId: sessionId, cwd: payload.projectPath })
      mainWindow?.webContents.send(IPC.PTY_BIND, earlyBindPayload)
      if (notificationWindow && !notificationWindow.isDestroyed()) {
        notificationWindow.webContents.send(IPC.PTY_BIND, earlyBindPayload)
      }
  ...
  ipcMain.handle(IPC.SESSION_RESUME, async (
    ...
    try {
      ...
      ptyManager.resumeSession({
        ...
        onExit: (sid, exitCode) => {
          console.log(`[PtyManager] Session ${sid} exited with code ${exitCode}`)
          sendToRenderers(IPC.SESSION_STATUS, { sessionId: sid, status: 'Completed' })
          // PTY 退出时直接向渲染进程发 PTY_UNBIND（同上，避免 claudeToPtyMap lookup 失败）
          const claudeId = ptyToClaudeMap.get(sid) ?? sid
          sendToRenderers(IPC.PTY_UNBIND, { ptyId: sid, claudeId })
          unbindPtyFromClaudeSession(claudeId)
          termWindows.get(sid)?.webContents.send(IPC.TERM_DATA, '\r\n[进程已退出]\r\n')
  ...
  ipcMain.handle(IPC.INSIGHT_RUN, async () => {
    ...
    let commandSent = false   // /insights 命令是否已发送（幂等守卫）
    let completed = false    // /insights 完成信号已触发
    let outputBuffer = ''    // 累积输出文本，用于提取文件路径

    /** 向 insight PTY 发送 /insights 命令（幂等：commandSent 守卫防止重复发送） */
    const sendInsightsCommand = (): void => {
      if (commandSent) return
      commandSent = true
      console.log('[insight] sending /insights command')
      ptyManager.rawWrite(insightId, '/insights\r')
    }

    /** 检测完成信号并触发通知（幂等：completed 守卫防止重复触发） */
    const handleCompletion = (): void => {
      if (completed) return
      completed = true
      // 从累积输出中提取 file:// 路径
      const fileMatch = outputBuffer.match(/file:\/\/[^\s"']+\.(?:html|json)/i)
      if (fileMatch) {
        const filePath = fileMatch[0].replace(/^file:\/\//, '')
        console.log(`[insight] report path: ${filePath}`)
        // 确保通知窗口已创建（按需自动打开）
        openNotificationWindow()
        // 延迟发送：等待通知窗口页面加载完成并注册 IPC 监听器
        // 参考 openNotificationWindow 中 catch-up 的 1s 延迟
        setTimeout(() => {
          sendToRenderers(IPC.INSIGHT_REPORT_READY, { filePath })
        }, 1000)
      } else {
        console.warn('[insight] completed but could not extract file path from output')
      }
    ...
    try {
      ptyManager.startBare(
        {
          ...
          onData: (_id, data) => {
            ...
            if (!commandSent) return  // /insights 未发送，跳过后续检测
            // 累积输出文本（保留最近 2KB 防止内存膨胀）
            outputBuffer += plain
            if (outputBuffer.length > 2048) outputBuffer = outputBuffer.slice(-2048)
            // 宽松多特征匹配完成信号（任一命中即视为完成）
            // Claude 输出包含多种可能的信号词，分 chunk 到达时可能只命中部分
            if (
              /insights?\s+(?:report|generated|ready)/i.test(outputBuffer) ||
              /\.html\b/.test(outputBuffer) ||
              /report\s+ready/i.test(outputBuffer) ||
              /generated\s+(?:your|the)/i.test(outputBuffer)
            ) {
              handleCompletion()
            }
          },
      ...
      // 定时器兜底：PTY 启动 15s 后（trust 已处理、Claude 已加载），若 onData 未检测到 prompt
      // 则主动发送 /insights。Claude TUI 的 * 光标常被 ANSI 转义序列包裹，stripAnsi 后不出现
      // 于 plain 文本，导致 onData 快速路径无法匹配。15s 充分覆盖 trust 对话框确认 + Claude 初始化。
      // sendInsightsCommand 幂等：若 onData 已发送则此处跳过。
      setTimeout(() => {
  ...
  // ── M10 独立通知窗口（单例，不设 parent，alwaysOnTop 可配置，关闭=隐藏到托盘）──
  ipcMain.handle(IPC.NOTIFICATION_WINDOW_OPEN, async () => {
    try {
      openNotificationWindow()
      return { ok: true }
    } catch (err) {
      console.error('[notification] NOTIFICATION_WINDOW_OPEN failed:', err)
      return { ok: false, error: String(err) }
    }
  })
...
async function startServices(): Promise<void> {
  ...
  const bus = createHookEventBus(
    () => [mainWindow, notificationWindow].filter((w): w is BrowserWindow => !!w && !w.isDestroyed()),
    HOOK_PORT
  )
    ...
    await startHookServer(HOOK_PORT, {
      onHookEvent: (payload) => {
        ...
        if (payload.hook_event_name === 'PermissionRequest') {
          ...
          // 缓存 payload，供通知窗口 catch-up（通知窗口加载完成时重放）
          recentPermissionPayloads.push(payload)
          if (recentPermissionPayloads.length > MAX_RECENT_PERMISSION) recentPermissionPayloads.shift()
          // 自动打开独立通知窗口（可配置默认开）
          const notifCfg = readDriverConfig()
          if (notifCfg.notifWindowAutoOpen !== false) {
            openNotificationWindow()
          }
        }
